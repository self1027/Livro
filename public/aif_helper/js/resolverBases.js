export function resolverBases({
	...
	// Base obrigatória
	tipoPessoa.BASE.bases.forEach(b => bases.push({
		...b,
		origem: "BASE"
	}))
	...
	atividades.forEach(opcao => {
		opcao.bases.forEach(b => bases.push({
			...b,
			origem: opcao.label
		}))
	})
	...
	const apenasObstrucao =
		totalSelecionados.length === 1 &&
		totalSelecionados[0].label === "Obstrução de fiscalização"
